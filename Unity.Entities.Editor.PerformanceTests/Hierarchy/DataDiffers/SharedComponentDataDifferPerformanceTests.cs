...
namespace Unity.Entities.Editor.PerformanceTests
{
    ...
    {
        [Test, Performance]
        public void SharedComponentDataDiffer_Spawn([Values(1000, 10_000, 100_000, 250_000, 500_000)] int entityCount)
        {
            ...
            Measure.Method(() =>
                {
                    var result = sharedComponentDataDiffer.GatherComponentChanges(World.EntityManager, query, World.UpdateAllocator.ToAllocator);
                    result.Dispose();
                })
                .SetUp(() =>
                {
        ...
        [Test, Performance]
        public void SharedComponentDataDiffer_NoChanges([Values(0, 1000, 10_000, 100_000, 500_000)] int entityCount)
        {
            var entities = CreateEntitiesWithMockSharedComponentData(entityCount, World.UpdateAllocator.ToAllocator, i => i % 100, typeof(EcsTestData), typeof(EcsTestSharedComp));
            var sharedComponentCount = World.EntityManager.GetSharedComponentCount();
            var query = World.EntityManager.CreateEntityQuery(typeof(EcsTestData));
            ...
            [Values(1000, 5000, 10_000)] int changeCount)
        {
            ...
            Measure.Method(() =>
                ...
                .SetUp(() =>
                {
                    ...
                    {
                        World.EntityManager.SetSharedComponentManaged(entities[i], new EcsTestSharedComp { value = counter++ % 100 });
                    }
                })
