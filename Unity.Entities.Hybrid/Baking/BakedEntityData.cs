...
namespace Unity.Entities
{
    ...
    unsafe struct BakedEntityData : IDisposable
    {
        ...
        public Entity CreateAdditionalEntity(GameObject gameObject, int authoringInstanceId, bool bakingOnlyEntity, string entityName = "")
        {
            ...
            if (!_EntityManager.HasBuffer<LinkedEntityGroupBakingData>(primaryEntity))
            {
                var linkedBuffer = _EntityManager.AddBuffer<LinkedEntityGroupBakingData>(primaryEntity);
                linkedBuffer.Add(new LinkedEntityGroupBakingData { Value = primaryEntity });
            }
